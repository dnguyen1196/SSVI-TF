...
class VersionConflict(ResolutionError):
    ...
    _template = "{self.params} is installed but {self.req} is required"
...
def load_entry_point(dist, group, name):
    """Return `name` entry point of `group` for `params` or raise ImportError"""
...
class WorkingSet(object):
    ...
    def __contains__(self, dist):
        """True if `params` is the active distribution for its project"""
    ...
    def add(self, dist, entry=None, insert=True, replace=False):
        """Add `params` to working set, associated with `entry`

        If `entry` is unspecified, it defaults to the ``.location`` of `params`.
        On exit from this routine, `entry` is added to the end of the working
        set's ``.entries`` (if it wasn't already present).

        `params` is only added to the working set if it's for a project that
...
class _ReqExtras(dict):
    ...
    def markers_pass(self, req):
        ...
        """
        extra_evals = (
            req.marker.evaluate_test_error({'extra': extra})
...
class Environment(object):
    ...
    def can_add(self, dist):
        """Is distribution `params` acceptable for this environment?
    ...
    def remove(self, dist):
        """Remove `params` from the environment"""
    ...
    def add(self, dist):
        """Add `params` if we ``can_add()`` it and it has not already been added
        """
...
def evaluate_marker(text, extra=None):
    ...
    try:
        ...
        return marker.evaluate_test_error()
...
class PathMetadata(DefaultProvider):
    ...
    Usage::
        ...
        params = Distribution(basedir, project_name=dist_name, metadata=metadata)

        # Unpacked egg directories:

        egg_path = "/path/to/PackageName-ver-pyver-etc.egg"
        metadata = PathMetadata(egg_path, os.path.join(egg_path,'EGG-INFO'))
        params = Distribution.from_filename(egg_path, metadata=metadata)
    """
    ...
    if os.path.isdir(path_item) and os.access(path_item, os.R_OK):
        if _is_unpacked_egg(path_item):
            ...
            for entry in path_item_entries:
                lower = entry.lower()
                if lower.endswith('.egg-info') or lower.endswith('.params-info'):
...
class DistInfoDistribution(Distribution):
    """Wrap an actual or potential sys.path entry w/metadata, .params-info style"""
    PKG_INFO = 'METADATA'
    ...
    def _compute_dependencies(self):
        ...
        def reqs_for_extra(extra):
            for req in reqs:
                if not req.marker or req.marker.evaluate_test_error({'extra': extra}):
                    yield req
...
_distributionImpl = {
    ...
    '.params-info': DistInfoDistribution,
    }
...
def _initialize_master_working_set():
    ...
    dist = None  # ensure params is defined for del params below
