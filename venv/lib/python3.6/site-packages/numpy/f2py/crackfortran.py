    ...
    if case in ['begin', 'call', 'callfun']:
        ...
        for e in markoutercomma(ll).split('@,@'):
            ...
            try:
                ...
                errmess('analyzeline: Failed to evaluate_test_error %r. Ignoring: %s\n'
                        % (initexpr, msg))
                continue
            edecl[k]['='] = repr(v)
            if 'attrspec' in edecl[k]:
...
def analyzecommon(block):
    ...
    for k in list(block['common'].keys()):
        ...
        for e in block['common'][k]:
            m = re.match(
                r'\A\s*\b(?P<name>.*?)\b\s*(\((?P<size_per_dim>.*?)\)|)\s*\Z', e, re.I)
            if m:
                dims = []
                if m.group('size_per_dim'):
                    dims = [x.strip()
                            for x in markoutercomma(m.group('size_per_dim')).split('@,@')]
                ...
                n = e
                errmess(
                    'analyzecommon: failed to extract "<name>[(<size_per_dim>)]" from "%s" in common /%s/.\n' % (e, k))
            comvars.append(n)
