...
# cppmacros['CHECKDIMS']="""\
# define CHECKDIMS(size_per_dim,rank) \\
...
# """
cppmacros[
    'ARRSIZE'] = '#define ARRSIZE(size_per_dim,rank) (_PyArray_multiply_list(size_per_dim,rank))'
cppmacros['OLDPYNUM'] = """\
#ifdef OLDPYNUM
...
static int initforcomb(npy_intp *size_per_dim,int nd,int tr) {
  int k;
  if (size_per_dim==NULL) return 0;
  if (nd<0) return 0;
  forcombcache.nd = nd;
  forcombcache.d = size_per_dim;
  forcombcache.tr = tr;
...
def buildcfuncs():
    ...
    cppmacros[
        m] = '#define %s(v,size_per_dim) (PyArray_New(&PyArray_Type, 1, size_per_dim, NPY_STRING, NULL, v, 1, NPY_ARRAY_CARRAY, NULL))' % (m)
